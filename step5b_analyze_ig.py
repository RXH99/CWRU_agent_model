...
理论依据（滚动轴承故障物理特征 — 已基于实际 IG 归因结果验证）:
  Normal:     无局部冲击，模型依靠全局平稳振动模式判断，归因均匀连续分布在整个信号上
  Inner Race: 内圈故障，冲击经转频调制，归因集中在带调制效应的脉冲簇上，局部聚焦但分布比外圈稍宽
  Outer Race: 外圈故障，固定位置冲击，等间隔块状集中红色条带，与周期性尖峰完全对齐
  Ball:       滚动体故障，冲击频率高、分布更分散，归因呈现在高频密集冲击集群区域
...
REFERENCE = {
    "Normal": """
理论特征: 无局部冲击，全局平稳振动
预期归因: 红线均匀连续分布在信号上，无明显局部集中
数值特征: pos_ratio 中等（~20%），peak_count 较高（>50，均匀分布导致多处局部凸起均为峰值）
活跃度:   broad
模式:     高峰值密度 + 低规则性（不是 no_peaks——均匀信号也有大量局部峰值）
""",
    "Inner Race": """
理论特征: 内圈故障，冲击经转频调制
预期归因: 红线集中在调制脉冲簇上，局部聚焦，比外圈稍宽
数值特征: pos_ratio 偏低（~11%，聚焦窄），peak_count 中等（~33），cr 高（~0.76）
活跃度:   focused 到 broad
模式:     random（因脉冲位置随调制变化，间隔不固定），自相关周期较长
""",
    "Outer Race": """
理论特征: 外圈故障，固定位置冲击，等间隔
预期归因: 等间隔块状集中红色条带，位置与周期性尖峰对齐
数值特征: pos_ratio 极低（~2%，归因仅集中在极窄脉冲处），peak_count 最低（~7），cr 极高（~0.95）
活跃度:   inactive 到 focused（归因范围极小，不代表无效）
模式:     峰极少但 cr 极高，说明归因集中在极少数强脉冲上
""",
    "Ball": """
理论特征: 滚动体故障，冲击频率高、分布分散
预期归因: 红线分布在高频、密集的冲击集群区域
数值特征: pos_ratio 较高（~25%），peak_count 最高（~100），cr 中等（~0.71）
活跃度:   broad
模式:     random（密集但不规律），峰值数远超其他类，反映高频特征
""",
}
...
def run():
    ...
    for cls in range(4):
        ...
        if not all_correct:
            ...
            all_ok = False
        elif CLASS_NAMES[cls] == "Normal":
            # Normal 的归因均匀分布是正常现象——模型依靠全局平稳模式做判断
            if pattern == "no_peaks":
                print("✅ 模型行为与正常样本物理特征一致：无局部冲击，全信号均匀关注")
            elif activity == "overactive":
                print("✅ 正常：模型均匀关注全信号（依靠全局平稳性判断正常）")
            else:
                print("✅ 模型行为与正常样本物理特征一致")
        elif activity == "overactive":
            print("⚠️ 分类正确，但归因范围太广，模型可能依赖全局统计而非局部特征")
        elif pattern == "no_peaks" and CLASS_NAMES[cls] != "Normal":
            print("⚠️ 分类正确，但归因缺乏冲击特征 - 故障样本应该有局部冲击响应")
        else:
            print("✅ 模型行为与故障物理特征一致")
